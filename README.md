# NERPA — Система управления поставками
...
```bash
# Настроить в .env:
# NERPA_BOT_TOKEN=токен от @BotFather
# NERPA_CHAT_IDS=ваш_chat_id (узнать: написать @userinfobot)
...
Переменные `NERPA_*` до ребрендинга назывались `TMS_*`. Старые имена всё ещё
читаются как запасной вариант (`app/env.py`), поэтому обновлять `.env` на сервере
можно не спеша — при использовании старого имени в лог уйдёт предупреждение.
...
| `NERPA_BOT_TOKEN` | Нет | Токен Telegram-бота |
| `NERPA_CHAT_IDS` | Нет | Получатели отчётов (через запятую) |
| `NERPA_DAILY_TIME` | Нет | Время ежедневного отчёта (HH:MM, по умолч. 20:00) |
| `NERPA_WEEKLY_TIME` | Нет | Время пятничного отчёта (HH:MM) |
| `NERPA_MONTHLY_TIME` | Нет | Время месячного отчёта (HH:MM) |
| `NERPA_CALLBACK_TIME` | Нет | Время напоминания о перезвонах (HH:MM) |
| `NERPA_HR_METRIC_REMIND_TIME` | Нет | Пятничное напоминание руководителям о метрике (HH:MM, по умолч. 12:00) |
| `NERPA_HR_METRIC_CHECK_TIME` | Нет | Пятничная сводка «кто не сдал метрику» (HH:MM, по умолч. 17:30) |
| `NERPA_TZ` | Нет | Часовой пояс бота (по умолч. Europe/Moscow) |
