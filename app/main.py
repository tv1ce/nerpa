...
from app.routers import auth, dashboard, counterparties, products, orders, invoices, contracts, settings, reports, warehouse, receivables, notifications, claims, activity, audit_log, board, logistics, leads, recon, field, files, public, sync_1c, sourcing
...
def _run_1c_sync_job():
    ...
    from app.services.onec_client import (
        ...
        sync_shipments_from_1c, sync_documents_from_1c,
    )
    db = SessionLocal()
    try:
        ...
        logger.info(
            "1C auto-sync: products c=%s u=%s; invoices c=%s u=%s; docs a=%s; payments u=%s",
