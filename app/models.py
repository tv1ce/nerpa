...
class CompanySettings(Base):
    ...
    # Через сколько часов простоя считать корзину брошенной и сказать менеджеру.
    # 0 или пусто — не напоминать.
    shop_abandon_hours = Column(Integer, default=2)
...
class ShopCart(Base):
    ...
    # Когда по этой корзине уже напоминали менеджеру. Сравнивается с updated_at:
    # клиент дособрал корзину и снова бросил — это новый повод напомнить, а вот
    # долбить одним и тем же каждые пятнадцать минут незачем.
...
class HrEmployee(Base):
    ...
    is_active = Column(Boolean, default=True)
    # Порядок в списке HR-учёта — как сотрудников перетащил HR. 0 у всех, пока
    # никто ничего не двигал: тогда список сортируется по алфавиту, как раньше.
