...
ботом, поэтому TMS-бот не видит строки «✅ <адрес> | 🟢 <время>», которые
публикует бот-помощник в группе перевозчика. Помощник дублирует их сюда HTTP-
вызовом, а TMS делает ровно то же, что делал бы по сообщению в чате.
...
Вместо `chat_id` можно указать `carrier_id` (id контрагента-перевозчика в TMS).
