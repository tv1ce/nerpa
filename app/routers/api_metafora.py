...
TMS находит заказ и переводит его в соответствующий статус. Спецификация для
...
Заказ ищется сначала по номеру TMS (надёжно), затем по адресу доставки среди
...
# Слово статуса → статус заказа TMS. Сравнение по вхождению, регистр не важен.
_STATUS_MAP = {
    "delivered": (
...
def _map_status(raw: str) -> str | None:
    """Слово перевозчика → статус TMS. None, если статус нам неизвестен."""
...
async def ping(request: Request):
    ...
    return JSONResponse({"ok": True, "message": "TMS на связи, ключ верный"})
...
async def status_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if not status:
        return JSONResponse({
            ...
            "message": f"Статус «{raw_status}» не распознан. Ожидаются «доставлено» "
                       ...
                       f"Отмену и возврат TMS по вебхуку не проводит — это делает менеджер.",
        })
    ...
    if not order:
        return JSONResponse({
            ...
            "message": "Заказ не найден: пришлите номер заказа TMS в поле order "
