...
поэтому NERPA только создаёт черновик документа с базовыми реквизитами —
...
async def create_invoice_edo(request: Request, invoice_id: int, db: Session = Depends(get_db)):
    ...
    заказу (из 1С), а не сгенерированный в NERPA. Данные (контрагент, номер, сумма)
...
# ── ЭДО: УПД (формализованный XML, собирается из заказа в NERPA) ────────────────
...
async def create_upd_edo(request: Request, order_id: int, db: Session = Depends(get_db)):
    ...
    собирается из данных заказа прямо в NERPA — без 1С. Черновик: подписание и
