...
async def save_modules(
    ...
    module_scripts:  str = Form(default=""),
    db: Session = Depends(get_db),
):
    company = db.query(CompanySettings).first()
    if not company:
    ...
    company.module_scripts  = bool(module_scripts)
    db.commit()
    ...
    request.session["mod_scripts"]  = company.module_scripts
...
async def save_metafora_api(
    ...
    """Токен API Метафоры — им NERPA создаёт заказы в системе перевозчика."""
    company = db.query(CompanySettings).first()
    if not company:
        company = CompanySettings()
        db.add(company)
...
async def bitrix_stock_push(request: Request, db: Session = Depends(get_db)):
    ...
    в CRM завели новые товары и их надо сопоставить с номенклатурой NERPA.
    ...
    именно так один клик по этой кнопке 06.08.2026 положил весь NERPA на
