...
двумя путями:
  ...
  • сообщением в группе перевозчика — его ловит NERPA-бот (`bot/main.py`);
...
NERPA-бот не увидит никогда — их нужно присылать напрямую в API.
...
def find_order_by_number(db: Session, number: str):
    """Заказ по номеру NERPA. «№80», «80», «0080» — всё это заказ №80.
