...
Заказ отправляется структурными полями (форма 3б), а не текстом: у NERPA все
поля уже разобраны, парсер Метафоры не нужен — быстрее и без риска, что он
что-то не так поймёт.

`external_id` — номер заказа NERPA. Он же ключ идемпотентности (повтор с тем же
...
def build_order_payload(order, attempt: int | None = None) -> dict:
    """Заказ NERPA → тело POST /orders (структурная форма).
