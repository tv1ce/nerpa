...
def sync_products_from_1c(db: Session) -> dict:
    """
    Читает Catalog_Номенклатура из 1С — создаёт/обновляет Products в NERPA.
    ...
    NERPA, а не только линкует существующие: пользователю нужна ПОЛНАЯ синхронизация
...
# «Склад», иначе в NERPA попадут и не-складские подразделения.
...
def sync_warehouses_from_1c(db: Session) -> dict:
    ...
    Warehouse в NERPA. В отличие от sync_products_from_1c — новые записи заводим
...
# ── Категории номенклатуры: 1С → NERPA ─────────────────────────────────────────
...
def push_order(order, db: Session) -> str | None:
    ...
    for i, item in enumerate(order.items, 1):
        ...
        if ext:
            ...
            zapasy.append({
                ...
                "Количество": qty,
                "Цена": price,
                # 1С требует явную «Сумма» (без неё ставит 0). УНФ-инвариант:
                # Сумма = Цена×Кол (до скидки), Всего = Сумма − Скидка (итог со скидкой).
                "Сумма": gross,
    ...
    comment_lines = [f"NERPA заказ #{order.number}"]
...
# ── Самовосстановление push заказов: NERPA → 1С ───────────────────────────────
...
# ── Оплаты: 1С → NERPA ─────────────────────────────────────────────────────────
# Счета из NERPA в 1С НЕ выгружаются. Счёт ведётся в 1С, NERPA только тянет статус
...
def sync_payments_from_1c(db: Session) -> dict:
    """
    Тянет оплату счетов ИЗ 1С (счета НЕ пушатся из NERPA) и разносит её через общий
    журнал платежей (app.utils.apply_payment/recompute_invoice_payment).
    Сопоставление: числовой номер счёта 1С == номер счёта NERPA И совпадение суммы.
    ...
    try:
        ...
        # 3. Разносим оплаты 1С по счетам NERPA через общий журнал платежей.
...
# ── Счета: 1С → NERPA (дубль счёта без печатных форм) ──────────────────────────
...
# контрагенту (прямой ссылки на заказ нет) — заказ в NERPA подбираем эвристически.
...
def _guess_order_for_invoice(db: Session, cp, contract, total: float):
    """Эвристика привязки счёта 1С к заказу NERPA (1С ссылку на заказ не хранит).
...
def sync_invoices_from_1c(db: Session) -> dict:
    """Тянет счета из 1С (Document_СчетНаОплату) и создаёт/обновляет их в NERPA.

    Идемпотентно по external_id_1c. Если в NERPA уже есть «ручной» счёт с тем же
    ...
    # Индексы NERPA по Ref_Key (один проход, без N запросов)
    ...
    for d in docs:
        ...
        if not cp:
            continue  # без контрагента в NERPA привязать счёт некуда
...
# ── Документы (Счёт PDF / УПД PDF / УПД XML): 1С → NERPA через HTTP-сервис ───────
# Печатные формы и XML отдаёт расширение 1С (HTTP-сервис tms на той же публикации,
# что OData). NERPA тянет файлы и прикладывает к заказу. Расходная накладная
...
def sync_shipments_from_1c(db: Session) -> dict:
    ...
    (→ СчетНаОплату → заказ через счёт в NERPA).

    Заодно дозаполняет в накладной пустой «Адрес доставки» из заказа NERPA: при
...
# ── Поступление товаров: 1С → NERPA (задача) + NERPA → 1С (подтверждение/проведение) ──
...
def sync_receiving_tasks_from_1c(db: Session) -> dict:
    ...
    удаляется и в NERPA; если его провели напрямую в 1С — задача помечается
    ...
    # (заявка отменена) или проведён напрямую в 1С, минуя кнопку «Принять» в NERPA.
    ...
    # реагирует на то, что уже существующая в NERPA запись сменила состояние.
...
# ── Складское перемещение: 1С → NERPA (задача) + NERPA → 1С (проведение) ────────
...
def sync_transfer_tasks_from_1c(db: Session) -> dict:
    ...
    Также сверяет уже загруженные задачи: если документ в 1С удалили — задача
    удаляется и в NERPA; если его провели напрямую в 1С — задача помечается done
...
def confirm_transfer(transfer, db: Session) -> dict:
    ...
    физически переместил товар — расхождений в этом контуре NERPA не запрашивает
...
# ── Списание: NERPA → 1С (создание + мгновенное проведение) ──────────────────
...
def push_writeoff(writeoff, db: Session) -> str | None:
    ...
    в том же запросе) — списание в NERPA всегда мгновенное, черновика не бывает.
...
# ── Остатки: 1С → NERPA (кэш реального остатка по складам) ───────────────────
...
def sync_stock_balances_from_1c(db: Session) -> dict:
    ...
    в кэш-таблицу StockBalance1C. Это единственное место в NERPA, где остаток
