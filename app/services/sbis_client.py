...
def _lookup_party_by_inn(inn: str) -> Optional[dict]:
    ...
    заполняется по актуальным данным, а не по тому, что могло устареть в NERPA)."""
...
def poll_sbis_edo_statuses(db) -> dict:
    ...
    СБИС.ПрочитатьДокумент и обновляет их в NERPA. При переходе в терминальный
