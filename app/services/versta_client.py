...
(контрагент «ООО Верста» в NERPA); здесь только чтение статуса/трекинга по
...
def poll_versta_statuses(db) -> dict:
    ...
    вручения). Если Versta считает заказ вручённым, статус заказа в NERPA
    ...
    # Пуш в Bitrix24 — после коммита, чтобы статус «Доставлено» в NERPA остался
