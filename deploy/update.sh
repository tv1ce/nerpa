...
APP_DIR="/opt/tms"
VENV="$APP_DIR/.venv"
REPO_DIR="/opt/tms"   # сервер держит git-репозиторий прямо в APP_DIR
...
# 1. Переключаемся на main и тянем
...
# 2. Обновляем зависимости (только если requirements изменились)
