...
|---|---|
| NERPA → 1С | Контрагенты, Заказы (создание + смена статуса), Счета (при выставлении) |
| 1С → NERPA | Статус оплаты счетов (`Invoice.status = 'paid'`, `paid_date`) |
| Склад | NERPA отдаёт остатки в 1С; движения `out` с `order_id` не пушатся (1С создаёт их сама через заказ) |
...
---
Продолжаем интеграцию NERPA с 1С:УНФ. Прочитай файл `docs/1c_resume.md` в проекте — там весь контекст. БД уже подготовлена (коммит c674d96). Следующий шаг — реализация OData-клиента (`app/services/onec_client.py`) и UI настроек. Адрес 1С: [ВСТАВЬ АДРЕС].
---
