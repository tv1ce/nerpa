# Интеграция TMS ↔ 1С:УНФ через OData
...
```
1С → TMS      Номенклатура (товары + цены)
              Оплаты счетов → обновление Invoice.status = 'paid'

TMS → 1С      Контрагенты (новые и изменённые)
              Заказы покупателей
              Счета на оплату
...
def sync_products_from_1c(db: Session) -> dict:
    """
    Читает Catalog_Номенклатура из 1С, обновляет Products в TMS.
...
| Конфликт нумерации документов | Хранить номер 1С отдельно, не перезаписывать номер TMS |
